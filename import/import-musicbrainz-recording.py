...
def main(args):
    ...
    for index, uuid in enumerate(args, start=1):
        ...
        rec = m.get_recording_by_id(uuid, ["artist-rels", "work-rels", "place-rels", "area-rels"])['recording']
        ...
        performance_list.append(uuid)

        perfid = "perf_" + uuid.rsplit('-', 1)[1]
...
def artist_participation(mb, perf):
    ...
    if len(mb["roles"]) > 0:
        ...
        retval += "MERGE (" + mb["artist"] + ")-[:PARTICIPATED_IN { roles: ['unknown']"
        if len(mb["instruments"]) > 0:
            retval += ", instruments: [" + ', '.join("'" + i.encode('utf-8').strip() +
                                                     "'" for i in mb["instruments"]) + "]"
        retval += " }]->(" + perf + ")\n"
